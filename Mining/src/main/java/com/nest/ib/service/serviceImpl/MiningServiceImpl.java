...
public class MiningServiceImpl implements MiningService {
    ...
    // 矿池合约地址
    private static String MINING_SAVE_CONTRACT_ADDRESS = null;
    ...
    // 报价需要的USDT数量
    private static BigInteger OFFER_USDT_AMOUNT = null;
    // 报价需要的ETH数量(wei)
    ...
    public void offer() {
        ...
        if (!START_MINING || !USER_WALLET_BALANCE_ENOUGH || OFFER_FACTORY_CONTRACT_ADDRESS == null || OFFER_PRICE_CONTRACT_ADDRESS == null || MINING_SAVE_CONTRACT_ADDRESS == null) {
        ...
        /**
         *   通过价格合约拿到最新的报价区块编号：
         *   1. 该方式拿到的是最后一个报价区块号，包含了吃单后的报价，而报价挖矿计算间隔区块时，不应该计算吃单后的报价（吃单后的报价不参与挖矿），所以在有人吃单报价的时候，设置的间隔区块数量会变大
         *   2. 建议：开发者使用数据库，将链上所有的报价合约都存储下来，筛除掉吃单后的报价，这样计算才准确
         */
        BigInteger latestOfferBlockNumber;
        NestMiningContract nestMiningContract = NestMiningContract.load(MINING_SAVE_CONTRACT_ADDRESS,web3j,credentials, Contract.GAS_PRICE,Contract.GAS_LIMIT);
        try {
            latestOfferBlockNumber = nestMiningContract.checkLatestMining().sendAsync().get();
        } catch (Exception e) {
            LOG.error("调用报价价格合约获取最后一笔报价区块号，出现异常");
            e.printStackTrace();
            return;
        }
        String transactionHash = sendUSDTOffer(web3j, credentials, gasPrice, nonce,blockNumber,latestOfferBlockNumber);
        ...
        while (true){
            try {
                ...
                */
                BigInteger nowLatestBlockNumber = nestMiningContract.checkLatestMining().sendAsync().get();
                ...
                */
                ...
                if(nowBlockNumber.compareTo(blockNumber) > 0){
                    ...
                    String hash = sendUSDTOffer(web3j, credentials, gasPrice, nonce, nowBlockNumber, latestOfferBlockNumber);
     ...
     */
    @Override
    public String sendUSDTOffer(Web3j web3j, Credentials credentials, BigInteger gasPrice, BigInteger nonce,BigInteger blockNumber,BigInteger latestBlockNumber) {
        ...
        if (b) {
            ...
            // 价格合约里面最新的有效价格
            BigDecimal otherMinerOfferPrice = new BigDecimal(bigIntegerBigIntegerBigIntegerTuple2.getValue2()).divide(UNIT_USDT,18,BigDecimal.ROUND_DOWN)
            ...
            // 当前交易所的价格
            BigDecimal companyOfferPrice = new BigDecimal(OFFER_USDT_AMOUNT).divide(UNIT_USDT,18,BigDecimal.ROUND_DOWN)
            ...
            // 报价USDT数量
            BigInteger offerUsdtAmount = OFFER_USDT_AMOUNT;
            if(otherMinerOfferPrice.multiply(new BigDecimal("1.1")).compareTo(companyOfferPrice) < 0
                ...
                offerUsdtAmount = OFFER_USDT_AMOUNT.multiply(new BigInteger("10"));
                payableEth = offerEthAmount.add(new BigInteger(String.valueOf(serviceCharge)));
            }
            LOG.info("报价ETH数量：" + offerEthAmount + " 报价USDT数量：" + offerUsdtAmount + "  打入合约ETH数量：" + payableEth);
            Function function = new Function(
                    "offer",
                    Arrays.<Type>asList(
                            new Uint256(offerEthAmount),
                            new Uint256(new BigInteger(String.valueOf(offerUsdtAmount))),
    ...
    */
    ...
    public void checkWalletBalance() {
        ...
        if(ethBalance.compareTo(PAYABLE_ETH_AMOUNT) <= 0 || usdtBalance.compareTo(OFFER_USDT_AMOUNT) < 0){
    ...
    */
    @Override
    public void updateMiningSaveContractAddress(String miningSaveContractAddress) {
        if(miningSaveContractAddress != null){
            MINING_SAVE_CONTRACT_ADDRESS = miningSaveContractAddress;
            System.out.println("矿池合约地址更新：" + MINING_SAVE_CONTRACT_ADDRESS);
        }
    }
    ...
    */
    ...
    public void updatePrice() {
        ...
        */
        if(OFFER_USDT_AMOUNT != null){
            if(usdtAmount.compareTo(new BigDecimal(OFFER_USDT_AMOUNT).multiply(new BigDecimal("1.2"))) > 0 || usdtAmount.compareTo(new BigDecimal(OFFER_USDT_AMOUNT).multiply(new BigDecimal("0.8"))) < 0){
                LOG.info("交易所价格出现了超过20%的偏差，已停止运行，请立即检查是否出错");
                START_MINING = false;
                return;
            }
        }
        OFFER_USDT_AMOUNT = new BigInteger(String.valueOf(usdtAmount.setScale(0,BigDecimal.ROUND_DOWN)));
        LOG.info("更新价格 --> 报价需要ETH数量为：" + OFFER_ETH_AMOUNT + "  USDT数量为：" + OFFER_USDT_AMOUNT);
    }
    /**
    * 获取交易所价格
    */
