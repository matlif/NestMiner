...
public class TheFirstUpdateTask implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        */
        ...
        try {
            offerFactoryContractAddress = mappingContract.checkAddress("nest.v3.offerMain").sendAsync().get();
            offerPriceContractAddress = mappingContract.checkAddress("nest.v3.offerPrice").sendAsync().get();
            miningSaveContractAddress = mappingContract.checkAddress("nest.v3.miningSave").sendAsync().get();
        } catch (Exception e) {
        ...
        if(!offerFactoryContractAddress.equalsIgnoreCase("0x0000000000000000000000000000000000000000") && !offerPriceContractAddress.equalsIgnoreCase("0x0000000000000000000000000000000000000000") && !miningSaveContractAddress.equalsIgnoreCase("0x0000000000000000000000000000000000000000")){
            ...
            miningService.updateMiningSaveContractAddress(miningSaveContractAddress);
            //对报价工厂合约进行USDT授权
            try {
                approveToOfferFactoryContract(web3j,offerFactoryContractAddress);
            } catch (Exception e) {
                LOG.error("对报价工厂合约进行USDT授权失败");
                e.printStackTrace();
                return;
            }
        }
    ...
    private void approveToOfferFactoryContract(Web3j web3j,String offerFactoryContractAddress) throws Exception {
        ...
        if(approveValue.compareTo(new BigInteger("100000000000000")) < 0){
            String transactionHash = erc20.approve(offerFactoryContractAddress, new BigInteger("999999999999999999999999999999999999999999")).sendAsync().get().getTransactionHash();
