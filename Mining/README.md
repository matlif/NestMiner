...
>
>报价涉及到的合约为：USDT代币合约，映射合约，报价合约，矿池合约，价格合约。
...
// 报价合约地址
String offerContractAddress = mappingContract.checkAddress("nest.v3.offerMain").sendAsync().get();
// 价格合约地址
String offerPriceContractAddress = mappingContract.checkAddress("nest.v3.offerPrice").sendAsync().get();
// 矿池合约地址
String miningSaveContractAddress = mappingContract.checkAddress("nest.v3.miningSave").sendAsync().get();
```

##### 3. 授权报价合约USDT

>报价需要将USDT转入到报价合约，转USDT操作是由报价合约调用USDT代币合约来执行，故需要对报价合约进行USDT授权。
...
* 方式一
  ...
  >通过调用矿池合约获取最后一笔报价的区块高度，通过链接节点获取最新的区块高度，相减即可得到间隔上一笔报价的间隔区块数量。
  >
  >优点：直接调用合约，简单，及时性高。
  >
  >缺点：吃单的同时也会进行报价，但是该报价是不参与挖矿，此时计算的挖矿区块间隔是错误的。
...
* 方式二
  ...
  >通过轮询最新的区块里面的所有交易，找到成功打包的报价交易。当前区块高度减去找到的最新报价区块高度，即为挖矿间隔的区块数量。
  >
  >优点：获取的挖矿间隔区块数量永远是正确的。将数据存储到数据库，进行取回报价资产操作时，可以很迅速的找到需要取回的合约地址。
  >
  >缺点：不断的轮询，对节点的访问频率变高，增加节点压力。当前区块高度和找到的最后一笔报价区块高度，有可能不是同一个时间点的数据，导致新的报价打包完成了，而没有检测到。
  ...
  for (EthBlock.TransactionResult tx : transactions) {
  	...
  	// 如果input长度小于10，那么说明只是普通转账，不涉及调用合约
      if(input.length() < 10){
      	continue;
      }
      // input字符串前10位是调用的合约方法名
      String substring = input.substring(0, 10);
      if(substring.equalsIgnoreCase("0xf6a4932f")) {
          String to = transaction.getTo();
          // 由于合约方法名有可能重复，检测被调用者是否是报价合约地址
  		if(to.equalsIgnoreCase(offerContractAddress)){
  			System.out.println("最新的报价区块高度为：" + nowBlockNumber);
  		}
  	}
  }
  ```
...
>通过交易频率很高的交易所API获取USDT-ETH价格，进行计算获取10ETH报价需要的USDT和ETH数量即可。如果价格偏离超过10%需要进行10倍报价，或者是进行多倍ETH报价，只需要在报价的时候乘以对应的倍数就行。
>
>部分交易所API需要海外节点才能访问，以下采用的是火币交易所API：

```java
// 火币USDT-ETH的API
String usdtEthPriceUrl = "https://api.huobi.pro/market/history/trade?symbol=ethusdt&size=1";
// 访问火币API获取的数据
String s;
try {
	s = HttpClientUtil.sendHttpGet(usdtEthPriceUrl);
}catch (Exception e){
	return null;
}
if(s == null){
...
// 10ETH报价需要的USDT数量
BigDecimal usdtAmount = exchangePrice.multiply(new BigDecimal("1000000")).multiply(ethAmount).setScale(0,BigDecimal.ROUND_DOWN);
```

##### 6. 发起报价交易

>如果当前报价的ETH/USDT价格与价格合约里面的最新生效价格偏离超过10%，那么报价资产需要10倍，报价支付的手续费不变（不需要10倍）。
>
>当设置的间隔区块满足要求，并且报价需要的ETH和USDT数量都已经计算出来，那么此时可以直接发起报价。

```java
// 报价需要的ETH数量
BigInteger ethAmount = "";
// 报价需要的USDT数量
BigInteger usdtAmount = "";
// USDT代币合约地址
...
// 获取USDT-ETH报价的最新生效价格(报价后过了25个区块没有被其他人置换资产)
Tuple2<BigInteger, BigInteger> bigIntegerBigIntegerBigIntegerTuple2 = nestOfferPriceContract.checkPriceNow(usdtTokenAddress).sendAsync().get();
// 最新生效价格
BigDecimal takeEffectPrice = new BigDecimal(bigIntegerBigIntegerBigIntegerTuple2.getValue2()).divide(UNIT_USDT,18,BigDecimal.ROUND_DOWN).divide(new BigDecimal(bigIntegerBigIntegerBigIntegerTuple2.getValue1()).divide(UNIT_ETH,18,BigDecimal.ROUND_DOWN),18,BigDecimal.ROUND_DOWN);
// 准备报价的价格
BigDecimal offerPrice = new BigDecimal(usdtAmount).divide(UNIT_USDT,18,BigDecimal.ROUND_DOWN).divide(new BigDecimal(ethAmount).divide(UNIT_ETH,18,BigDecimal.ROUND_DOWN),18,BigDecimal.ROUND_DOWN);
// 报价需要支付的手续费(1%)
...
if(takeEffectPrice.multiply(new BigDecimal("1.1")).compareTo(offerPrice) < 0 || takeEffectPrice.multiply(new BigDecimal("0.9")).compareTo(offerPrice) > 0){
	...
	usdtAmount = usdtAmount.multiply(new BigInteger("10"));
